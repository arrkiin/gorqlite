...
PeerLoop:
	for peerNum, peerToTry := range peersToTry {
		...
		if err != nil {
			trace("%s: got error '%s' doing retryablehttp.NewRequest", conn.ID, err.Error())
			failureLog = append(failureLog, fmt.Sprintf("%s failed due to %s", url, err.Error()))
			continue PeerLoop
		}
		trace("%s: retryablehttp.NewRequest() OK")
		req.Header.Set("Content-Type", "application/json")
		client := retryablehttp.NewClient()
		client.HTTPClient.Timeout = time.Duration(conn.timeout) * time.Second
...
PeerLoop:
	for peerNum, peer := range peersToTry {
		...
		for responseStatus == "Haven't Tried Yet" || responseStatus == "301 Moved Permanently" {
			...
			req, err := retryablehttp.NewRequest("POST", url, bytes.NewBuffer(jStatements))
			if err != nil {
				trace("%s: got error '%s' doing retryablehttp.NewRequest", conn.ID, err.Error())
				failureLog = append(failureLog, fmt.Sprintf("%s failed due to %s", url, err.Error()))
				continue PeerLoop
